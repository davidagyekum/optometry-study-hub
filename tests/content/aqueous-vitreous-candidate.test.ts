...
describe('canonical Aqueous and Vitreous candidate bank', () => {
  it('has the declared draft-only integrity and exact blueprint', () => {
    ...
    expect(bank.questions).toHaveLength(80); expect(bank.objectives).toHaveLength(13); expect(new Set(bank.questions.map((q) => q.id)).size).toBe(80);
  ...
  it('preserves the exact ordered semantic content of the original 36 questions', () => {
    const preserved = aqueousVitreousCandidateBank.questions.slice(0, 36);
    expect(preserved.map((question) => question.id)).toEqual([
      ...
      'aqueous-chambers-sba-002',
      'aqueous-angle-sba-001',
      'aqueous-chambers-mr-001',
      'aqueous-chambers-ordering-001',
      'aqueous-chambers-hotspot-001',
      'aqueous-production-sba-001',
      'aqueous-barrier-sba-001',
      'aqueous-production-mr-002',
      'aqueous-production-matching-001',
      'aqueous-production-ordering-001',
      'aqueous-flow-sba-002',
      'aqueous-flow-extended-001',
      'aqueous-iop-sba-001',
      'aqueous-iop-sba-002',
      'aqueous-iop-mr-001',
      'aqueous-iop-matching-001',
      'vitreous-anatomy-sba-001',
      'vitreous-structure-sba-001',
      'vitreous-anatomy-ordering-001',
      'vitreous-anatomy-matching-001',
      'vitreous-anatomy-hotspot-001',
      'vitreous-anatomy-label-001',
      'vitreous-clinical-sba-001',
      'vitreous-clinical-sba-002',
      'vitreous-clinical-extended-001',
      'vitreous-clinical-short-001',
      'vitreous-clinical-open-response-002',
    ]);
    expect(createHash('sha256').update(JSON.stringify(preserved)).digest('hex'))
      .toBe('3575c6033d05c5593af6c50e19aeb6ccecff55c51fb6d5a2f082d649715db6b9');
  });  it('uses accessible structured data for every table stimulus', () => {
    const tables = aqueousVitreousCandidateBank.questions.filter((q) => q.stimulusType === 'table'); expect(tables).toHaveLength(7);
  ...
  it('meets candidate content-quality rules', () => {
    ...
    expect(newQuestions).toHaveLength(71);
