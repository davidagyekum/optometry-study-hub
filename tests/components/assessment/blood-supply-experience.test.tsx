...
describe('Blood Supply curated experience', () => {
  beforeEach(() => {
  ...
  it('shows the generic study entry while preserving the legacy quiz action', () => {
    window.history.replaceState({}, '', '/study/blood-supply');
    render(<StudyApp />);
  ...
  it('opens curated practice from the study-page entry', async () => {
    ...
    expect(window.location.pathname).toBe(
      '/practice/blood-supply-curated',
    );
  });
  it('loads the generic route lazily and starts a valid Quick 10 session', async () => {
    window.history.replaceState({}, '', '/practice/blood-supply-curated');
    render(<StudyApp />);
    expect(await screen.findByRole('heading', {
      name: 'Curated slide-aligned practice',
    }, { timeout: 5_000 })).toBeInTheDocument();
  ...
  it('fails closed without importing the learner experience when disabled', () => {
    ...
    expect(screen.queryByText(
      'Which artery is the usual parent vessel of the ophthalmic artery?',
