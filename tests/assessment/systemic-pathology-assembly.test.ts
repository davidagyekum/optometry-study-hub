...
function count(
  questions: typeof systemicPathologyCandidateBank.questions,
...
describe('Systemic Pathology deterministic fixed profiles', () => {
  it('satisfies every hard profile contract across 1,000 seeds', () => {
    for (let seedIndex = 0; seedIndex < 1_000; seedIndex += 1) {
      for (const profileId of ['quick', 'standard', 'full'] as const) {
        ...
        const selection = createSystemicPathologyPracticeSelection({
          profileId,
          ...
          seed: `systemic-pathology-seed-${seedIndex}`,
        });
        const first = assemblePractice({
          ...
          sectionFormatAvailability:
            SYSTEMIC_PATHOLOGY_PROFILE_SECTION_FORMAT_ALLOCATIONS[profileId],
        });
        if (!first.ok) {
          throw new Error(
