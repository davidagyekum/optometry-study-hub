...
function count(
  questions: typeof autonomicPharmacologyCandidateBank.questions,
...
describe('Autonomic Pharmacology deterministic fixed profiles', () => {
  it('satisfies every hard profile contract across 1,000 seeds', () => {
    for (let seedIndex = 0; seedIndex < 1_000; seedIndex += 1) {
      for (const profileId of ['quick', 'standard', 'full'] as const) {
        ...
        const selection = createAutonomicPharmacologyPracticeSelection({
          profileId,
          ...
          seed: `autonomic-pharmacology-seed-${seedIndex}`,
        });
        const first = assemblePractice({
          ...
          sectionFormatAvailability:
            AUTONOMIC_PHARMACOLOGY_PROFILE_SECTION_FORMAT_ALLOCATIONS[profileId],
        });
        if (!first.ok) {
          throw new Error(
