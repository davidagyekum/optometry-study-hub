...
export const extended = (meta: Meta, stems: [string, string][], options: Entry[], correctAnswers: Record<string, string>): AssessmentQuestion => ({ ...base(meta), format: 'extended_matching', stems: stems.map(([id, text]) => ({ id, text })), options: entries(options), correctAnswers });
