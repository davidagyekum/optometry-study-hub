...
responses, current versions only, and no relaxation.
...
pure and deterministic. Insufficient targeted pools produce explicit
availability diagnostics instead of silently changing strategy.
...
- encounter, partial, incorrect, unanswered, and manual-required counts;
...
`attemptCount` advances only when a response was supplied. Outcome counters
follow the verified grade. Old records do not receive invented historical
breakdowns; new counters begin from the first PR 10 result.
...
stores a result and removes its active attempt. A collision, snapshot mismatch,
grading failure, history-version conflict, malformed history record, or final
StoreV2 validation failure commits nothing. A repeated finalization cannot
double count. The Aqueous engineering pilot uses the default disabled history
policy.
...
same controlled session UI. Submitted answers produce `manual_required`
outcomes and no percentage. Result review displays the canonical explanation,
sample answer, and rubric. Written practice may record encounter/manual-review
history but never increments correctness.
