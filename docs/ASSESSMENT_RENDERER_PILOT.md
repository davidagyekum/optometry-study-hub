...
Making a formerly complete draft incomplete removes its committed response but retains the draft. Clearing removes both. Historical attempts without `draftResponses` remain valid. When a valid draft exists, resolution also requires its format-derived complete response to be absent for incomplete work or semantically equivalent for complete work; `DRAFT_RESPONSE_MISMATCH` rejects divergent snapshots before compatibility, submission, or grading. Malformed persisted drafts return structured resolution errors and are never silently repaired.
...
Resume, direct attempt/result routes, active selection, latest-result selection, and submission verify the exact pilot blueprint, course, module, nine-question set, current versions, Study mode, stored presentation order, `diagnostic@1`, complete responses, and drafts. Incompatible snapshots are never silently repaired or discarded. Candidate-aware selection retains exact-blueprint pilot snapshots and reports compatibility issues separately. The landing and direct-attempt recovery views let the learner confirm either a blueprint-guarded discard or an atomic replacement with a fresh exact pilot. Multiple active pilot candidates are reported explicitly, and unrelated assessment attempts and results remain untouched.

## Submission and grading

Submission summarizes answered, in-progress, unanswered, and flagged counts. Opening the confirmation focuses and announces its heading; cancelling restores focus to Submit. Question navigation uses automatic scrolling when reduced motion is requested and smooth scrolling otherwise. A pure issue partition keeps question-specific draft/response validation beside its originating control with `aria-describedby` and polite announcement, while controller, compatibility, and storage failures appear in the session `role=alert`. Navigation hides unrelated validation, and successful retries clear the relevant error. All failures preserve the active attempt. It warns that incomplete drafts grade as unanswered and that an answered open response requires manual review.
