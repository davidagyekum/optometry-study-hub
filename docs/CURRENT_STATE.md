...
Tissue Foundations and Ocular Adnexa now join the authored Notes V3 catalog with all nine stable reading IDs intact. Their 14,393 words retain labelled source qualifications, course corrections, comparison tables, focus maps, one-minute summaries, active recall and existing cleared figures. A typed answer-free loader registry keeps all four authored manuscripts in separate route-specific chunks; load or validation failure still returns the unchanged Notes V2 content. StoreV2, both storage keys, 680 curated questions, 400 legacy questions, assessment history, feature defaults and deployment profiles are unchanged. See `docs/NOTES_V3_BATCH2.md`.
## Notes V3 Batch 3
